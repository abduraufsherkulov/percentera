...
export class ListOfCreditsIn extends Component {
  constructor(props) {
    super(props);
    this.state = {
      percent: +"",
  ...
  componentDidMount() {
    ...
    const endpoint = "https://hidden-oasis-96512.herokuapp.com/getBankCredit";
    ...
    axios({
      ...
      .then(response => {
        console.log(response);
        this.setState({
          amount: response.data[0].credit_amount,
          long: response.data[0].credit_long,
          given: response.data[0].date,
          credit_type: response.data[0].interest_type,
          percent: +response.data[0].credit_interest,
          bank_name: response.data[0].bank_name
        });

        //investor + percentera percentage
        let overall = this.state.percenteraFee + this.state.percent;
    ...
    var listItems = (
      <React.Fragment>
        ...
        <div className="col-4">
          ...
          <div>
            <label>Percentera's fee</label>
            <p>{this.state.percenteraFee} %</p>
          </div>
          <div>
            <label>Daily amount must be paid</label>
        ...
        <div className="col-4">
          <div>
            ...
            <p>
              {this.state.long}{" "}
              {this.state.credit_type === "Daily" ? "Days" : "Months"}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="down-p">{this.state.bank_name}</p>
          </div>
        </div>
      </React.Fragment>
    );
